...
model = RandomForestClassifier(n_estimators=200, min_samples_leaf=5, max_depth=20, random_state=42)
model.fit(X_train, y_train)

y_prob = model.predict_proba(X_test)

perm_importance = permutation_importance(model, X_test, y_test)
feature = ['Weather', 'Latitude', 'Longitude', 'Vessel_type', 'Tons', 'Month', 'Hour']
plt.barh(feature, model.feature_importances_)
plt.show()
...
def recommend_top_3_accident_types(Weather, Latitude, Longitude, Vessel_type, Tons, Month, Hour):
    ...
    for column in ['Weather', 'Vessel_type']:
...
recommended_accident_types = recommend_top_3_accident_types(sample_weather, sample_latitude, sample_longitude, sample_Vessel_type, sample_tonnage, sample_month, sample_hour)
